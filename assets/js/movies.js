document.addEventListener('DOMContentLoaded', () => {
    ...
    searchInput.addEventListener('input', () => {
        ...
        if (query.length < 3) {
            searchResults.innerHTML = ''; // Clear results if less than 3 characters
            searchResults.style.display = 'none';
            fetchMoviesByGenres(); // Reload full list if input is empty or less than 3 characters
            return;
        }
    ...
    if (defaultGenreCheckbox) {
        ...
        fetchMoviesByGenres(); // Initial call to load movies
    }
